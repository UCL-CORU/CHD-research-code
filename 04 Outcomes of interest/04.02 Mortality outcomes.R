...
# 2. We only need a few fields from nchda: identifiers, ages, and processed life status
  ...
  summary( nchda_data$raodis )
  length( which(nchda_data$raodis<0) )
...
# 3. In-hospital mortality
  ...
  sum( nchda_data$ageofdeath < nchda_data$raop, na.rm = T ) #DANGER: in some records, ageofdeath is incompatible with age at procedure
  # we overwrite ageofdeath to avoid danger
  nchda_data$ageofdeath[ nchda_data$ageofdeath < nchda_data$raop ] = NA
  ...
  length( sel2 ) 
  table( nchda_data$death_inhosp[ sel[ sel2 ] ], useNA="always" ) #most of those already well recorded
  ...
  table( nchda_data$death_inhosp, useNA = "always" ) # in-hospital death unknown for few records
  
  length( unique( nchda_data$championpatid[ is.na(  nchda_data$death_inhosp )]) ) # those correspond to even fewer patients 
  
# 4. 30-day mortality
  #we follow the same hierarchy: deaths have preference, then NCHDA and then ONS life status
  summary( nchda_data$ageofdeath ) #note that this definition is at patient level
  
  THIRTY_DAYS = 30.0 / 365.25 
  ...
  table( nchda_data$death_30d, useNA = "always" ) 
  ...
  length( sel2 ) 
  table( nchda_data$death_30d[ sel[ sel2 ] ], useNA="always" ) #most of those already well recorded
  ...
  sel2 = which(nchda_data$ons_age_life_status[ sel ] > THIRTY_DAYS + nchda_data$raop[ sel ] )
  length( sel2 ) #it is really important to get life status and not only deaths
  nchda_data$death_30d[ sel[ sel2 ] ] <- 0
  gc()
  
  table( nchda_data$death_30d, useNA = "always" ) # 30-day death unknown for x records
  
  length( unique( nchda_data$championpatid[ is.na(  nchda_data$death_30d )]) ) # those correspond to y patients 

# 5. 90-day mortality
  #we follow the same hierarchy: deaths have preference, then NCHDA and then ONS life status
  NINETY_DAYS = 90.0 / 365.25 # a quarter of a year
  ...
  table( nchda_data$death_90d, useNA = "always" ) 
  ...
  sel2 = which(nchda_data$ons_age_life_status[ sel ] <= NINETY_DAYS + nchda_data$raop[ sel ] & 
                 nchda_data$raop[ sel ] <= nchda_data$ons_age_life_status[ sel ] )
  length( sel2 )
  table( nchda_data$death_90d[ sel[ sel2 ] ], useNA="always" ) #most of those already well recorded
  ...
  table( nchda_data$death_90d, useNA = "always" ) # 90-day death unknown for x records
  
  length( unique( nchda_data$championpatid[ is.na(  nchda_data$death_90d )]) ) # those correspond to y patients 
