...
source(paste0(dir_COARfile_champion,"/", CHDtype," modules.R")) # load all COA specific functions 
...
if(inclusionexlcusion==1){
  ...
  NCHDAdata= COA_inclusion_and_exclusion_module(NCHDAdata)
  
  # derived variable.   marker of COA patients
  table(NCHDAdata$COApat[which(NCHDAdata$patentry==1)],useNA="ifany")
  
  
  ########################### save the NCHDA dataset
  write.csv(NCHDAdata,dir_NCHDAdataset,row.names = F)
  #  the COA dataset (raw data)
  
  # further exclusion:  remove patients with more complex CHD which may not be fully captured by exclusion codes 
  COAdata_raw=NCHDAdata[which(NCHDAdata$COApat==1 & NCHDAdata$TGApat==0  & NCHDAdata$PApat==0 & NCHDAdata$AVSDpat==0 & NCHDAdata$TOFpat==0 & NCHDAdata$AOSpat==0),]
  ...
  COAdata_raw=COAdata_raw[which(!(COAdata_raw$patid %in% HLHS$patid | COAdata_raw$patid %in% FUH$patid)),]
  COAdata_raw$CHDtype=CHDtype

  write.csv(COAdata_raw,dir_data_raw,row.names = F)
}
...
#################################################
if(pathway==1){
  #load the main dataset for COA
  COAdata_raw=loadmaindata(dir_data_raw)
  ########################### run pathway module

  COAdata_raw=COA_pathway_module(COAdata_raw)
  # derived variable.  
  # category of each procedure (record level)
  table(COAdata_raw$CatProcLabel,useNA="ifany") # category of each procedure in COA, 2: SV stage two"and 3: SV stage three (Fontan) will be empty
  ...
  table(COAdata_raw$ProcSeq[which(COAdata_raw$patentry==1)],useNA="ifany")
  
  
  
  ########################### diagnosis subtype  
  COAdata_raw=COA_diagnosis_subtype_module(COAdata_raw)
  
  # derived variable.  
  table(COAdata_raw$diagsubgroupLab[which(COAdata_raw$patentry==1)],useNA="ifany")
...
if(FP==1){
  ...
  COAdata_raw=Generic_violation_module(COAdata_raw)
  
  #COA specific violation rule:
  COAdata_raw=COA_specific_violation_module(COAdata_raw)
  
  #important var. pat_violation: patients who need to removed according to the violation rule
  table(COAdata_raw$pat_violation[which(COAdata_raw$patentry==1)],useNA="ifany")
  table(COAdata_raw$pat_violationinf[which(COAdata_raw$patentry==1)])  # the reason for violation
  ...
  COAdata_raw=Generic_suspected_missing_miscoded_data_module(COAdata_raw)
  
  #COA specific suspected rule:  NONE
  
  table(COAdata_raw$Flagcentre_missingdata[which(COAdata_raw$patentry==1)],useNA="ifany")
  table(COAdata_raw$Flagcentre_missingdatainf[which(COAdata_raw$patentry==1)]) # the reason 
  
  
  #### minor data errors/unusual records 
  #generic rule:  minor data error if patients had bypass surgery as pre-pathway
  
  COAdata_raw=Generic_minor_data_errors_module(COAdata_raw)
  
  #COA specific minor data errors rule:  
  
  COAdata_raw=COA_specific_minor_data_errors_module(COAdata_raw)
  
  table(COAdata_raw$Flagcentre_minor[which(COAdata_raw$patentry==1)],useNA="ifany") # marker of Flagcentre_minor
  table(COAdata_raw$Flagcentre_minorinf[which(COAdata_raw$patentry==1)]) # the reason 
  ...
  COAdata_final=COAdata_raw[-which(COAdata_raw$pat_violation==1),]
  write.csv(COAdata_final,dir_data_final,row.names = F)
}
...
if(descriptiveanalysis==1){
  #load the main dataset for COA
  COAdata_final=loadmaindata(dir_data_final)
  
  COAdata_final=Generic_descriptive_analysis(COAdata_final)
  
 #another pathway for CoA only
  COAdata_final<-COAdata_final %>%
    group_by(patid)%>%
    mutate(pathasVSDpathway=max(CatProc==5)) %>%
    ungroup()
  ...
  table(COAdata_final$patpremat[which(COAdata_final$patentry==1)],useNA="ifany") 
...
if(PatientLevelData==1){
  ...
  COApatientleveldata=Generic_save_patient_level_data_module(COAdata_final)
  write.csv(COApatientleveldata,dir_dataPatientLevel,row.names = F)
...
######################################################################################
if(metrics==1){
  COApatientleveldata=read.csv(dir_dataPatientLevel,colClasses = c(ProcSeq= "character"))
  
  # compute survival cumulative incidence rates at age 1-year, 5-years and 10-years (using Kaplan-meier)
  survialrates(COApatientleveldata,ages=c(1,5,10))
  ...
  # all reintervention
  Reintrates(status=COApatientleveldata$Status_allReint,timeofstatus=COApatientleveldata$timeofStatus_allReInt)
